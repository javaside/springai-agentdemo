...
| 消息列表 | 用途 | 如何表示图片 |
| --- | --- | --- |
| **会话消息列表** | 跨请求记录用户消息、大模型回复和工具结果 | 只保存 `file_reference`，不保存 `Media` |
| **本次请求消息列表** | Spring AI 根据会话历史、当前输入和工具循环消息组装，作为本次 `Prompt.messages` 发给大模型 | code-tui 在底层模型调用前临时加入 `Media` |

两者的关系是：

```text
会话历史 + 当前输入 + 工具循环消息
  -> Spring AI 组装本次 Prompt.messages
  -> code-tui 复制 Prompt.messages
  -> 根据本次需要处理的 file_reference 读取图片
  -> 在消息副本中加入 Media
  -> 将处理后的 Prompt.messages 交给底层模型
```

code-tui 不是先把图片写进会话、发送后再删除，而是从一开始就不让 `Media` 进入会话。带图片的消息副本只传给本次底层模型调用，不写回会话；不存在请求结束后再清理会话图片的步骤。
...
会话消息列表中的当前 UserMessage
  ...
  -> 在本次请求的消息副本中写入 media
  -> 引用状态改写为实际结果，如 delivery = delivered
...
这里补上 `media`，是为了让大模型真正收到图片。`delivery` 改写为真实状态，是为了不向模型发送自相矛盾的信号：图片已发送时，引用不应仍写着“当前未发送”。

处理只改变本次请求中的消息副本，会话中的 `UserMessage` 仍然是 `media = 空`、`delivery = not_in_view`。图片发送失败或超出预算时，`delivery` 会改写为对应状态，具体规则见第 10 节。
