 ...
 *   <li><b>已知例外 1（如实声明）</b>：cursor 工厂的一次性成本（diff 的读文件 + LCS，
 *       O(一个工具入参)，受 DiffRenderer 的 LCS_MAX/BODY_CAP 上限约束）无法按段切片，发生在
 *       第一段之前、时间预算之外——每条 diff 输出只付一次。详见
 *       {@code PhysicalOutputQueue} 类注释；</li>
 *   <li><b>已知例外 2（如实声明）</b>：markdown <b>表格块</b>的攒块与排版。scrollback 只能追加、
 *       印出去改不了，而列宽必须按整块的真实内容算，所以表格行只能先<b>缓冲</b>、块结束才一次排出。
 *       与例外 1 不是同一个模式：排版发生在<b>批中途某个 {@code next()} 内部</b>，缓冲还
 *       <b>跨游标、跨批次、跨 OutputLine 存活</b>且归属渲染器（上面「状态跨调用保持」说的是 O(1) 的
 *       围栏态，不含内容缓冲）。量级由渲染器的输入上限（200 行 / 64 K 原文字符，越限即降级成逐行
 *       原样输出）与排版的产出上限（600 物理行，越限整块退回原样）双向封顶。
 *       另有一条白捡的豁免要知道：{@code drain} 的时间预算检查要 {@code written >= 2} 才生效，
 *       而缓冲期间不写行——所以一批可以喂完 600 条 pending + 300 条流式行而完全不受 12ms 约束；
 *       量级同样由上面两个上限封顶。详见 {@code ScrollbackPrinter.md} 与表格渲染设计 §3.5/§3.6；</li>
