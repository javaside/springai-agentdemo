...
- **权限层自身的已知弱点**（诚实列出，别高估它）：
  ...
  - **命令判定是尽力而为**：含命令替换 `` ` ``/`$()`、进程替换、引号不配对等**拆不动**的命令一律退回人工确认（失败关闭），但规则 DSL 不做空白归一、路径匹配区分大小写（macOS APFS 上 `/etc` 与 `/ETC` 是同一文件，deny 方向会漏）、也不解析符号链接。**deny 规则是尽力而为，真正的护栏是不可绕过的内置底线。**
...
- **命中内置底线时只有 3 项**（去掉「本会话/永久」）：那两项在此处是谎言——任何 allow 规则都消不掉内置底线的询问。
- 建议规则刻意取**最窄**形态：
