...
- **回合中插话**：回合跑到一半时输入的消息，**不用等整个回合结束**。直接 Enter 即可，不打断回合、不取消已经跑对的部分——消息随**下一次模型调用**送达，等待从「剩余整个回合」缩短到「当前这一个工具」。一个回合可能有十几轮工具调用、好几分钟，而人在工具循环中途开口十有八九是看它跑偏了想纠正；等事都做完再听，这句话就从「纠偏」降级成了「善后」。未送达的钉在输入框上方（`⤷ 你的原话`）看得见，送达那一刻才随信息流滚动；按 Esc 取消回合时，没送出去的插话会**放回输入框**而不是丢弃。想排到下回合再发用 `/queue <消息>`。压缩中、以及回合已被取消只剩子 agent 收尾时会自动回落成排队（那两种状态不会再有模型调用，插话进去等于石沉大海）。详见下方「回合中插话」。
...
- **生产日志最低级别固定为 `INFO`**：源码与发布包均不使用 `DEBUG` / `TRACE`；`WARN`、`ERROR` 仍按严重程度保留。日志只写文件，不设 CONSOLE appender，避免 stdout/stderr 撕裂 TUI。
