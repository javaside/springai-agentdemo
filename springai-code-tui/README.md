...
支持视觉的模型能**真正看见**工具产生的图片：`Read` 一张 png、MCP 截图工具的返回，都会作为图片交给模型，而不只是一行文件路径。

> ### ⚠️ 本期不支持用户直接贴图
>
> **你在输入框里写 `docs/bug.png` 不会自动附图**——输入框还不认识图片路径，拖拽、路径识别、附件行都**尚未实现（期 2）**。
> 兑现机制的两条路径（用户图 / 工具图）代码里都已实现，但**只有工具那条被接上了**。
> 所以本期模型能看见的图**只能来自工具**：让它 `Read docs/bug.png`，或让 MCP 截图工具产出。
...
- **终端里显示不了图片**：模型截的图、生成的图，你在 TUI 里只会看到一行文件路径，要看得自己去开文件。
...
- **视觉输入本期只到「工具产图」**（详见上方「视觉输入」）：**用户不能直接贴图**（输入框不认识图片路径，拖拽/附件行属期 2）；**终端里显示不了图片**（只会看到一行文件路径）；**`Bash` 生成的图片文件不会自动产生引用**（有意为之，模型想看就 `Read` 它）；**视频恒不投递**。另外，消息序列只在**一个本地兼容网关 + `gpt-5.6-sol`** 上真机验证过，`api.openai.com` 原生端点与 Anthropic / 千问 / 智谱三家**完全未验证**，第一次真用仍可能 400。
