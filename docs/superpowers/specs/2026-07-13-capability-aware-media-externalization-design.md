...
> 核心原则（**已按 #9 收敛，读本行即可**）：清出会话的只有**模型读不懂的字节**——图片/视频/二进制。**文本文件正文一直留在会话**（模型的工作材料，Edit 精确匹配、跨回合续用都靠它）。图片/视频/二进制在「产生它的那一回合」就当场塌成引用（字节从不进非视觉模型）；文本永不外置。下方首版设计写的是「所有文件内容都塌成引用」——**那是错的，以本行 + 修正表 #9 为准**。
...
| 2 | §5/§7「路径②超 **32KB 阈值**才外置文件读取」 | ~~改为：能反查到 in-root 文件 → 一律外置，不看大小~~ **此条已被 #9 取代**（那版把文本文件也引用化，事后发现有害）。当前：见 #9。 |
...
| 9 | 「所有文件（含**文本文件**）内容都不驻留会话，一律外置为引用」（#2 的强化版） | **文本文件引用化有害，已回退。** 文本是模型的工作材料：跨回合把它换引用 → 下回合 `Edit` 的 `old_string` 无法逐字符匹配（模型记不住确切空白）、续用只能盲写、还得重读把省下的 token 花回去。**最终规则（无任何阈值）**：①**文本文件** → 不外置，原文留会话；②**图片/视频/二进制文件** → 引用；③**非文件（Bash 输出等，路径不可反查）** → 不动。判据是磁盘文件魔数（`MagicSniffer.isTextFile`，Tika），非返回串长相。图片/二进制引用化是纯收益（模型读不懂字节）；文本留会话的代价（长会话累积占用）交给上下文压缩管，不在此处激进引用化。 |

（评审流程扎实但**输入的 MCP 契约是假的**，害得全部下游射偏；根因是手写带 `type` 的 fixture 骗过了所有单测。修复时所有回归用例改用**线上真实无 type 串**，并验证「修复前必红」。另：总目标一度被误读成"连文本也不能留"，见 #9 更正——真正要清出会话的只有模型读不懂的字节。）
...
② 回合间兜底——只清「非文本文件」（**文本文件永不动**，见修正表 #9）
   submit() 开头，遍历已存在历史事件（过往回合）
     → 找到「文件读取」的 ToolResponseMessage（能从 ToolCall.arguments 反查到 in-root 文件）
     → 磁盘文件魔数判：文本文件 → 放行不动（原文留会话）；图片/视频/二进制 → 换引用
     → 非文件（反查不到路径，如 Bash 输出）→ 不动
   本回合工具循环内的读：不动，全文在途
```
> ⚠️ 本框②的标题在首版写作「文本文件内容——回合间外置」，**已废**：文本文件不再外置。②如今只是 ①（即时媒体外置）的**回合间兜底**——万一某个非文本文件读取绕过了①（如 Read 把 PNG 读成文本），下一回合在这里补引用。
