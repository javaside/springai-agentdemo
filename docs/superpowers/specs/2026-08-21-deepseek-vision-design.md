...
> **决策记录（2026-08-21 补充，用户已拍板保持现状）**：这里「默认 inline」的论证曾漏掉一笔
> 真实的账——**同一回合内每次工具迭代都会重发请求**（无状态请求的固有代价），内联意味着同一张
> 图的字节被反复送，而 files 通道上传一次拿 `file_id`、后续迭代只引用 id，能在**轮内迭代**这一层
> 省下真实传输量（请求体从 ~MiB 级降到几十字节）。
>
> 澄清两个边界，避免以后重复讨论：
> - **files 不省钱**：DeepSeek 图片按「每图最多 384 token」计费，与传输方式无关（base64 内联与
>   file_id 引用账单相同）。files 省的是带宽/延迟与请求体大小，不是费用。
> - **files 的价值建立在「文件被反复引用」之上**：本项目图片字节不进会话存储（「当轮兑现」设计），
>   跨轮不累积——但**轮内迭代重传是真实存在的复用场景**，files 恰好覆盖它。这正是 files 在本系统
>   里真正的价值点，与「拒绝跨轮累积」不矛盾。
>
> 已知局限（未修）：`file_id` 当前只活在进程内（`DeepSeekFileStore` sha 缓存），不持久化；跨轮
> 引用同一张图仍会重新上传（上传一次即永久有效，理论上可把 `file_id` 写进引用块跨轮/跨会话复用，
> 改动引用块结构与兑现链路，本期不做，按需再议）。
