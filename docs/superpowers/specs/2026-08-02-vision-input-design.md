...
### 期 2：用户贴图（UI 密集，需 pty 实机）—— ✅ 已完成（2026-08-02）

输入框路径识别、附件行渲染、能力闸门与输入保留、`-c` 回放引用块渲染、~~`MATERIALIZED` 快照语义~~。

> 细化设计见 [用户贴图设计](2026-08-02-vision-user-attachments-design.md)，实施见
> [期 2 实施计划](../plans/2026-08-02-vision-user-attachments.md)。
> **快照语义那条被推翻**：最终是「项目内指原文件、项目外才复制」，理由见该设计 §5（同 §8 表格那行）。

下列「期 2 待解问题」在期 2 落地时的实际结论：

- **误附识别**：倾向规则被采纳并加了第四条——**路径独立成词 + 相对路径以 project root 为基准 + 文件真实存在 + 魔数是图片**。但**误附没有被消灭**：`把 docs/bug.png 复制到 tmp/` 四条全中，仍然会附。代价改由**附件行实时可见 + `Ctrl+G` 撤销**承担
- 路径形态：反斜杠转义空格（macOS 终端拖拽默认形态）、单/双引号包裹、`~` 展开、中文名、多路径，均已支持
- 相对路径基准：**project root**，与原计划一致
- 异常文件：不满足判据一律**当普通文本**，不报错也不提示
- 撤销附件：**改为专门交互 `Ctrl+G`**——原计划的「改输入文本即可」不够用，误附时你要删的正是那句话里的路径
- 被能力闸门拦住后**输入框内容原地保留**，与原计划一致
