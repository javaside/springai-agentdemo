...
void calculateColumnWidths_findsMaxWidthPerColumn() {
    ...
    assertArrayEquals(new int[]{19, 6}, widths); // "codetui.syncOutput"=19, "参数"=4
}
...
void reduceColumnWidths_stopsAtIterationLimit() {
    ...
    // 超过 10000 次迭代，返回原数组
    assertArrayEquals(new int[]{65000}, reduced);
}
```
    ...
    return new OutputCursor() {
        ...
        public PhysicalLine next() {
            ...
            if (!wrapped.isEmpty()) {
                ...
                // raw = 整块的多行 Text（所有 flushed 行共享同一 raw）
                Text raw = Text.of(flushed.stream()
                    .map(spans -> spans.stream().map(Span::content).collect(Collectors.joining()))
                    .collect(Collectors.joining("\n")));
                
                return new PhysicalLine(withIndent, raw);
            }
...
- [ ] **Step 1: 在 enqueueOutputLine 的 USER/TOOL_START 分支前置 flush cursor**

在 `:516` 附近的 `USER` case 中，入队前添加：

```java
case USER:
    // 前置 flush cursor（无条件入队）
    outputQueue.add(printer.tableFlushCursor());
    // ... 既有代码
    break;
```

对 `TOOL_START` / `TOOL_OK` / `TOOL_FAIL` / `ERROR` / `SUBAGENT_*` case 做相同处理。

- [ ] **Step 2: INFO 豁免不 flush**
...
- [ ] **Step 1: 在主 drain 之后（:978-984 之后）添加兜底 flush**

在 `computeFollowUpFlags` 之后、continuation 调度之前插入：

```java
// 兜底 flush：回合结束或模态时排空缓冲
if ((isIdle() || hasModal()) && inputDrained() && printer.hasBufferedTable()) {
    outputQueue.add(printer.tableFlushCursor());
    drainQueuedOutput(remainingNanos);
}
...
### Task 17: CodeTuiView flush 触发点接线（第 5 条：printPlan 收尾）
...
在 `:3237` 附近，逐行 enqueue 之后追加：

```java
// 整篇 ASSISTANT 文档灌完，收尾 flush
outputQueue.add(printer.tableFlushCursor());
```

- [ ] **Step 2: 在 /clear 处理中同步丢缓冲**

在 `:2161` 附近，调用 `printer.reset()` 之前添加：

```java
// 同步丢缓冲（不能放进 runOnRenderThread）
md.reset();
```
...
Run: `mvn test -pl springai-code-tui -Dtest=CodeTuiViewEventWiringTest`
Expected: 至少一个测试 FAIL（表格在 USER 行前消失）

恢复代码。

- [ ] **Step 2: 变异测试 - 删除「输入已排空」判定**

修改第 4 条 flush 触发点，去掉 `inputDrained()` 条件。

Run: `mvn test -pl springai-code-tui -Dtest=CodeTuiViewEventWiringTest#largePendingBatch*`
Expected: FAIL（半张对齐 + 半张原样）

恢复代码。

- [ ] **Step 3: 变异测试 - ERROR 加入豁免**

将 `ERROR` case 改为不 flush（像 INFO 一样）。

Run: `mvn test -pl springai-code-tui -Dtest=CodeTuiViewEventWiringTest`
Expected: FAIL（错误消息排在表格前面）

恢复代码。

- [ ] **Step 4: 变异测试 - 注掉 next() 内部循环**

将 `MdLineCursor.next()` 中的 `while` 改成 `if`。

Run: `mvn test -pl springai-code-tui -Dtest=ScrollbackPrinterTest#streamingLinesCursor_handles200LineTableBatch`
Expected: FAIL（200 行批次丢失）

恢复代码。

- [ ] **Step 5: 变异测试 - 注掉削列迭代上限**

将 `reduceColumnWidths` 中的 `MAX_ITERATIONS` 检查去掉。

Run: `mvn test -pl springai-code-tui -Dtest=MarkdownTableTest#reduceColumnWidths_stopsAtIterationLimit`
Expected: FAIL（极端输入超时）

恢复代码。

- [ ] **Step 6: 变异测试 - 注掉产出侧 600 行上限**

将 `render` 中的 `MAX_OUTPUT_LINES` 检查去掉。
...
`ScrollbackPrinterTest:54` 和 `:303` 的期望需要调整（一次性 `assistant(String)` 改走 feed + flush）。
...
```bash
git commit -am "test(code-tui): 修正既有测试期望值（一次性入口改走 feed+flush）"
```
