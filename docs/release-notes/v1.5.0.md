...
在 [v1.4.0](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.4.0.md) 基础上的**功能版**（minor）。核心交付物仍是终端编码智能体 **`springai-code-tui`**。本版三大新增：**联网搜索**（博查 + Brave 两家共存，模型按内容语言自选）、**MCP 远程传输**（Streamable HTTP，可连 Context7 一类远程 server）、**Claude Opus 5 接入并设为 Anthropic 默认模型**。无破坏性变更，建议所有用户升级。
...
> 完整功能全景与⚠️安全声明见 [v1.3.0 发布说明](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.3.0.md) 与 [v1.0.0 发布说明](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.0.0.md)；本文只列出相对 v1.4.0 的变化。
...
[Apache License 2.0](https://github.com/javaside/springai-agentdemo/blob/main/LICENSE)。发布包内随附 `LICENSE` 与 `NOTICE`（含所分发第三方库：Spring AI / Spring Boot / spring-ai-community 为 Apache 2.0，TamboUI 为 MIT）。本版未引入新的第三方依赖——两个搜索工具与 MCP HTTP 传输均复用既有依赖树（`spring-web`、`spring-ai-agent-utils`、`mcp-core`）。
