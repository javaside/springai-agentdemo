...
在 [v1.3.1](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.3.1.md) 基础上的**功能版**（minor）。核心交付物仍是终端编码智能体 **`springai-code-tui`**。本版三大新增：**通义千问 provider**（含流式工具调用分片修复）、**`*_MODELS` 可配置模型清单**、**`/mcp` 运行期 MCP 管理**（启用/禁用即时生效 + 配置回写）。无破坏性变更，建议所有用户升级。
...
> 完整功能全景与⚠️安全声明见 [v1.3.0 发布说明](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.3.0.md) 与 [v1.0.0 发布说明](https://github.com/javaside/springai-agentdemo/blob/main/docs/release-notes/v1.0.0.md)；本文只列出相对 v1.3.1 的变化。
...
设计与实施记录见 [`docs/superpowers/specs/2026-07-16-mcp-runtime-management-design.md`](https://github.com/javaside/springai-agentdemo/blob/main/docs/superpowers/specs/2026-07-16-mcp-runtime-management-design.md)。已知边界：管理粒度为整 server；新增/删除条目或改 `command`/`args` 仍需重启（面板条目集在启动期定型）。
