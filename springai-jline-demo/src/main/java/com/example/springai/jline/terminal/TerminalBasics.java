 ...
 * Terminal 基础用法（先看这个）。
 *
 * <p>这是一个【读源码就能学会】的例子——没有分屏、没有样式框架、没有抽象。
 * {@link #run} 里从上到下线性地把 Terminal 最常用的几件事各做一遍，每步都有注释，照着抄即可。</p>
 *
 * <p>关于「创建」与「关闭」：Terminal 由启动器统一负责，本类直接用传入的 terminal、不自行创建/关闭。
 * 启动器里的写法就是标准做法（try-with-resources，离开 try 自动 close() 复原）：</p>
 * <pre>
...
public final class TerminalBasics implements Demo {
    ...
    public void run(Terminal terminal) throws java.io.IOException {
        ...
        // ⑧ 收尾：原始模式下读一个键即返回菜单（无回显）。Terminal 的 close() 由启动器负责。
        terminal.writer().println("\n基础演示结束，按任意键返回菜单。");
        terminal.flush();
        Attributes prev3 = terminal.enterRawMode();
        try {
            terminal.reader().read();
        } finally {
            terminal.setAttributes(prev3);
        }
    }
}
