...
| 版本 | 类型 | 摘要 |
| --- | --- | --- |
| [v1.17.2](docs/release-notes/v1.17.2.md) | 修订版 | **终结「输出中打字」卡死与终端崩溃**：token 估算移出渲染线程（大会话不再周期性冻结）、流式残行设 20 万字符上限（杜绝百万字符单行每帧处理）、渲染降频（tick 100ms / 下沉 66ms）+ 残行预览节流 150ms（输出中 ANSI 频率大降，冲击 Terminal.app 输入法合成的概率随之大降） |
