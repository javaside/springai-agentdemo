...
4. **resize 拖拽终值重排**（Task 7 删除宽度轮询兜底后的唯一无自动覆盖残余风险——自动 PTY 冒烟只覆盖单次 TIOCSWINSZ 定步 resize，不覆盖连续拖拽的多次 SIGWINCH 连发）：拖拽窗口宽度多次（宽↔窄快速连拖后停住），确认停稳约 150ms（132ms settle 窗口）后界面按**最终**宽度重排——输入框圆角边框贴合新宽度、scrollback 干净重放无残影/错位；若终端卡在旧宽度（边框仍按旧宽度画、或 scrollback 里留下旧帧残骸）即为该风险的复现信号；
5. **持续时长与规模**：记录总运行时长（建议 ≥5 分钟连续输出+输入）、窗口数（2）、输出规模（行数/字节量级）；
6. **观察项**（逐项记录）：
...
7. **若 Terminal.app 崩溃**：保存 crash report（系统会弹「Terminal 已意外退出」→ 查看/导出，或 `~/Library/Logs/DiagnosticReports/Terminal-*.ips`），并检查栈中是否出现：GCD kevent 相关帧、`setMarkedText:`、`selectedRange`、`NSTextInputContext`、`IMKInputSession`——这些是历史崩溃的特征路径；把 report 一并反馈。
