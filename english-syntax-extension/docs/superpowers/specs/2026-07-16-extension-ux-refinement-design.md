...
**补充(实现期修订)**:running(未完成)与 paused 状态下,主按钮下方额外显示一个次级「恢复网页原文」按钮(STOP_SESSION)。原设计只在完成态由主按钮承担恢复,但 `isSessionComplete` 要求所有已发现句子都解析完——页面下方有未滚动到的句子时完成态不可达,而 running/paused 的主按钮必须保留暂停/继续语义(防重复机制),故恢复入口需独立存在。次级按钮样式弱化、与主按钮共用 in-flight 禁用;主按钮已是「恢复网页原文」(完成态)时不显示。
