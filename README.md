...
| **回合中插话** | 回合跑到一半时输入的消息**不用等它跑完**——直接 Enter 即可，不打断回合、不丢弃已跑对的部分，消息随下一次模型调用送达（等待从「剩余整个回合」缩到「当前这一个工具」）。`/queue` 可显式排到下回合；Esc 取消时未送达的插话**放回输入框**而非丢弃 |
| **界面** | 单栏对话式 TUI：流式输出、工具活动行、📋 计划面板、⟐ 任务面板、状态栏（含未送达插话计数） |
